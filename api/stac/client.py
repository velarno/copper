...
class AsyncStacClient(httpx.AsyncClient):
    ...
    async def fetch_catalog_links(self) -> List[Dict[str, Any]]:
        response = await self.get(self.catalogue_url)
        data = response.json()
        links = data.get("links", [])
    ...
    async def fetch_all_collections(self):
        ...
        tasks = {
            "collections": [self.fetch_collection_from_url(link) for link in collection_links],
            "parameters": [self.fetch_collection_inputs_from_id(collection_id) for collection_id in collection_ids]
        }
        ...
        with Session(engine) as session:
            for collection_data in collections:
                ...
                if collection_data.retrieve_inputs:
                    input_schema = InputSchema.create_with_parameters(collection_data.retrieve_inputs, collection)
                    entities.append(input_schema)
