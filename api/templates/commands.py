...
def list(
    ...
    limit: Optional[int] = typer.Option(
        None, "--limit", "-l", help="Limit the number of templates to list"
    ),
    children: Optional[str] = typer.Option(
        None, "--children", "-c", help="List children of a template"
    ),
):
    if children:
...
def add(
    ...
    parameter_value: Optional[str] = typer.Option(
        None, "--value", "-v", help="Parameter value"
    ),
    parameter_range: Optional[str] = typer.Option(
        None, "--range", "-r", help="Parameter range, format: from-to"
    ),
):
...
def show(
    ...
    compact: bool = typer.Option(
        False, "--compact", "-C", help="Compact output (overrides indent)"
    ),
    with_metadata: bool = typer.Option(
        False, "--metadata", "-M", help="Include template metadata in the output"
    ),
):
    ...
    try:
        ...
        raise typer.Exit(1)
    console.print_json(
        template_updater.to_json(indent=indent, with_metadata=with_metadata)
    )
...
@app.command(
    name="parameters", help="Show the parameters of a template (aliases: params, par)"
)
def parameters(
    template_name: str = typer.Argument(..., help="Template name"),
    hide_values: bool = typer.Option(
        False, "--hide-values", "-V", help="Hide parameter values"
    ),
    format: OutputFormat = typer.Option(
        OutputFormat.json, "--format", "-f", help="Output format"
    ),
    ...
    match format:
        case OutputFormat.json:
            console.print_json(
                models_to_json(template_updater.allowed_parameters(hide_values))
            )
        case OutputFormat.table:
...
def cost(
    template_name: str = typer.Argument(..., help="Template name"),
    method: CostMethod = typer.Option(
        CostMethod.local, "--method", "-m", help="Cost method"
    ),
):
    template_updater = TemplateUpdater(template_name)
...
@app.command(
    name="history", help="Show the history of a template (aliases: log, hist, history)"
)
def history(
    template_name: str = typer.Argument(..., help="Template name"),
):
...
@app.command(
    name="download",
    help="Download a dataset using a template (aliases: dl, get, fetch, retrieve)",
)
def download(
    template_name: str = typer.Argument(..., help="Template name"),
    output_dir: Path = typer.Option(
        default_output_dir, "--output-dir", "-o", help="Output directory", dir_okay=True
    ),
):
    template_updater = TemplateUpdater(template_name)
    client = CdsClient()
    client.retrieve(template_updater.dataset_id, template_updater.to_dict()).download(
        output_dir
    )
...
def export(
    ...
    output_file: Path = typer.Option(
        ..., "--output-file", "-o", help="Output file", dir_okay=False
    ),
...
@app.command(
    name="import", help="Import a template from a JSON file (aliases: ingest, load)"
)
def load(
    ...
    input_file: Path = typer.Option(
        ..., "--file", "-f", help="Input file", dir_okay=False
    ),
):
...
@app.command(
    name="budget",
    help="Split a template into smaller templates to meet a budget",
    hidden=True,
)
@app.command(
    name="optimize",
    help="Split a template into smaller templates to meet a budget (aliases: optim, opt, split, budget)",
)
def optimize(
    template_name: str = typer.Argument(..., help="Template name"),
    ...
    if dry_run:
        ...
        console.print(
            f"Template {template_name} would have been split into {len(optimizer.valid)} templates"
        )
    else:
        optimizer.persist_templates()


# TODO: doesn't work yet because template required parameters are not checked
# TODO: create an automatic check for required parameters
@app.command(name="dl", hidden=True)
@app.command(
    name="download", help="Download a dataset using a template (aliases: dl, download)"
)
def _download(
    template_name: str = typer.Argument(..., help="Template name"),
    output_dir: Path = typer.Option(
    ...
    budget: float = typer.Option(
        ...
        "--budget",
        "-b",
        help="Default Budget to use if value not available in template",
    ),
):
    template_updater = TemplateUpdater(template_name)
    client = CdsClient()
    cost_estimate = template_updater.cost

    if cost_estimate > budget:
        console.print(
            f"Template {template_name} would cost {cost_estimate} which is more than the budget of {budget}"
        )
        raise typer.Exit(1)
    else:
        client.retrieve(
            template_updater.dataset_id, template_updater.to_dict()
        ).download(output_dir)
        console.print(f"Downloaded {template_updater.dataset_id} to {output_dir}")
